...
document.getElementById("boltSelectionButtonAddID").addEventListener("click", lastBoltkitToTableFunc);


function boltGostFill () { // Заполняем выбор гостов
    let i = 0;
    for (let key in bolt){
...
function boltDiametrFill () { // Заполняем диаметр от гостов
...
function boltReСount () { //пересчитываем болт
selectGOSTIndex = document.getElementById("gostID").options.selectedIndex;
selectGOST = document.getElementById("gostID").options[selectGOSTIndex].text;
selectboltIndex = document.getElementById("boltID").options.selectedIndex;
selectbolt = document.getElementById("boltID").options[selectboltIndex].text;
...
function lastBoltkitToTableFunc (){ //Добовялем подобранный болткомплект в таблицу
