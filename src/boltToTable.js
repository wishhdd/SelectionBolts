...
export function lastBoltkitToTable (lastBoltKit) { // добвляем болт комплект в таблицу
...
function addCell (alignTrue){ // добавляем ячейку обрамлением
...
function boltToTable (lastBolt) { // Добавляем болт в таблицу
    ...
    addCell(true);
    newCell.appendChild(document.createTextNode(`${Math.round((1 * lastBolt.ourBolt.weightX1000/1000))/1000}`));
    addCell(true);
...
function nutToTable(ourNut){ // Добавляем  гайки
    newRow = table.insertRow(-1);
    newCell = newRow.insertCell(0);
...
function flatWasherToTable (lastFlatWasher) { // добавляем плоскую шайбу
    newRow = table.insertRow(-1);
    newCell = newRow.insertCell(0);
    newCell.setAttribute("colspan", "3");
    newCell.setAttribute("data-b-a-s", "thin");
    ...
    addCell (true);
    newCell.appendChild(document.createTextNode(`${Math.round(( lastFlatWasher.weightX1000/1000))/1000}`));
    addCell (true);
...
function springWasherToTable (lastSpringWasher) { // добавляем пружинную шайбу
    newRow = table.insertRow(-1);
    newCell = newRow.insertCell(0);
    newCell.setAttribute("colspan", "3");
    newCell.setAttribute("data-b-a-s", "thin");
    ...
    addCell(true);
    newCell.appendChild(document.createTextNode(`${Math.round(( lastSpringWasher.weightX1000/1000))/1000}`));
    addCell(true);
...
function boltsTableExport() { // Экспортируем табилцу в экель
    inputToTable();
